...
class NiftiDataset(data.Dataset):
    def __init__(self, root_dir, transform_info_file, resize=None):
...
#        scout_nii_path = os.path.join(self.root_dir, patient_info['pathOrigScoutnii'])
        scout_nii_path = os.path.join(patient_info['Paths']['data'])
        transform_matrix = np.array(patient_info['A']['Augmented_A']) # transform_matrix = np.array(patient_info['TransformSurvToSA']['A'])

        # Load NIFTI data
        scout_nii = sitk.ReadImage(scout_nii_path)
        scout_data = torch.FloatTensor(sitk.GetArrayFromImage(scout_nii))
        if scout_data.shape[0] > 182:
            scout_data = scout_data[:182, :, :]
        if scout_data.shape[1] != 244:
            scout_data = scout_data[:, :244, :244]
...
#     # Example usage:
#     root_dir = r'D:\Dataset\Data'
#     transform_info_file = os.path.join(root_dir, "validInfo.json")
#
#     dataset = NiftiDataset(root_dir=root_dir, transform_info_file=transform_info_file, resize=0.5)
